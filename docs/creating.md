...
A LowTalk dialogue is one thing: an NPC's conversation, made of passages, lines, options and the commands that make
...
7. **Dialogue...** edits the settings: which NPCs open it (with a picker of roles and tags), speaker, title,
   start passage, whether it opens when a player joins, portrait and shared memory.
8. **Test here** plays your unsaved draft from the passage you are on, with this NPC. **Save** writes the file where
   it lives and reloads it. **Discard** throws the draft away.

Limits: the editor works one passage at a time, so there is no overview of the whole conversation; the Node Editor
...
3. The editor shows a form: the NPCs it binds to (with autocomplete of roles and tags), speaker, title, start
   rules, and a list of passages. Each passage holds statements, each with its own fields: a Say has a speaker and text,
...
passage per stretch of conversation, statements wired into bodies, options and branches fanning out. What it saves
...
`sh tools/nodeeditor/install.sh` (or `powershell -ExecutionPolicy Bypass -File tools\nodeeditor\install.ps1` on Windows) when the editor stops offering `LowTalk - Dialogue`. The Windows one is untested; please report it if it fails.
