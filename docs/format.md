...
| `<<attitude friendly>>` | Set this NPC's attitude toward the player: ignore, hostile, neutral, friendly, revered. This decides who the NPC will **start** on, not a fight already under way: setting a goblin friendly while it is swinging at you does not call it off, and the game offers no way to. Only a role with a hostile branch does anything with `hostile` at all — most peaceful NPCs, including the temple-template villagers, have no such branch and will not react. |
