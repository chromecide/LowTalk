...
| `<<attitude friendly>>` | Set this NPC's attitude toward the player: ignore, hostile, neutral, friendly, revered. This decides who the NPC will **start** on, not a fight already under way — for that, follow it with `<<calm>>`. Only a role with a hostile branch does anything with `hostile` at all; most peaceful NPCs, including the temple-template villagers, have no such branch and will not react. |
| `<<calm>>` | Make this NPC forget whatever it was fighting, by clearing the targets its role has marked. Use it with `<<attitude friendly>>` to call an NPC off: the attitude stops it choosing you again, and this stops the attack it is already making. A role that never marks a target does nothing and reports no error. |
...
| `<<input $var "Prompt">>`, `<<input $var "Prompt" number>>` | Show a text box and store what the player types. **`number` makes the box come back until they type one**, rather than storing a word that ends the conversation in whichever later line first treats the answer as a number. The kind goes after a quoted prompt. |
| `<<once>>` ... `<<endonce>>` | The block between runs at most once per player. |
| `<<random>>` ... `<<or>>` ... `<<endrandom>>` | One alternative runs, chosen at random. |
| `<<wait 2>>` | Pause that many seconds before what follows. The line before it shows without a Continue button and the next line appears by itself. Keep it short (0 to 30); the player can still Leave. **Anything after a wait runs only if the conversation is still open**, so do not put something that has to happen — putting an NPC back to how it was, say — after one. If the player leaves, or the window closes for any other reason, the rest of the passage is abandoned. |
...
| `<<title "Main" ["Small"] [Style] [seconds]>>` | A cinematic title across the screen. The game draws the second line small and **above** the main one, the way it announces a zone, so `<<title "Howling Sands" "you have entered">>` reads "you have entered" over "Howling Sands". The style and the seconds are recognised in any position: `<<title "Chapter One" Major 5>>`. The styles are the game's own, not this mod's: every version has `Default` and `Major`, and 0.7 adds `GoblinBreach` and `VoidEviction`. `/lowtalk reference` lists what your server has. `minor` still reads as `Default`, and on a server too old to have styles anything but `Default` shows as the louder treatment, because that is as much as its call can say. |
...
| `<<vfx Particle_System_Id [scale] [seconds] [player]>>` | Play a particle effect where the conversation is: on the NPC or prop that is speaking, or at the block a dialogue is bound to. Add `player` to put it on the player instead. With neither an NPC nor a block, it is on the player anyway. |
