...
public final class JsonDialogues {
    ...
    /**
     * The part of a data set worth putting in a dropdown: entries containing the typed text, best matches first,
     * trimmed to a limit. A list of thousands is not sent to the client, so the longest ones are narrowed here as
     * the creator types. Entries that start with the text come before ones that merely contain it, and when
     * nothing matches the start of the whole list is returned so the dropdown is never empty.
     */
